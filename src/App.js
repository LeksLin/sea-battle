...
function App() {
  ...
  const [shipsState, setShipsState] = useState([1]);
  ...
  const buttonClick = () => {
    ...
    setBoolState(false);
    randomShips();
    console.log(ships, shipsForbidden);
  }
...
const addShipsForbidden = (shipsForbidden, valueArr, index) => {
  ...
  return (
    <div className="App">
      <button onClick={buttonClick} type="button">Клик</button>
      <PoleConclusion boolState={boolState} index={index} ships={shipsState} />
    </div>
  );
}
